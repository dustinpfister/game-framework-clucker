...
********** ********** *********/
    ...
    // the given sheetKey, and imageIndex in stack.images. A cellIndex
    // is an array of cellInfo objects, or a function that will 
    // produce such an array
    api.createSpriteSheet = function(stack, sheetKey, imageIndex, cellIndex){
        var spriteSheet = {
            name: sheetKey,
            image: stack.images[imageIndex],
            cells : []
        };
        ...
        if(typeof cellIndex === 'function'){
           spriteSheet.cells = cellIndex.call(stack, spriteSheet.image, spriteSheet, stack);
        }
        stack.spriteSheets[sheetKey] = spriteSheet;

    };
    // built in method to help create a sprite sheet object in the typical grid layout
    api.createSpriteSheetGrid = function(stack, sheetKey, imageIndex, cellWidth, cellHeight){
        cellWidth = cellWidth === undefined ? 32: cellWidth;
        cellHeight = cellHeight === undefined ? 32: cellHeight;
        api.createSpriteSheet(stack, sheetKey, imageIndex, function(image, spriteSheet, stack){
            ...
            while( cellIndex < cellCount ){
                cx = cellIndex % cellWidth;
                cy = Math.floor(cellIndex / cellWidth);
